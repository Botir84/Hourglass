...
// RX=5 (ishlatilmaydi), TX=3 (bu pin Bluetooth-ning RXD piniga boradi)
 ...
 */
int getGravity() {
  mpu6050.update(); // Datchikdan yangi raqamli ma'lumotni olish
  
  float x = mpu6050.getAngleX();
  float y = mpu6050.getAngleY();

  // MPU6050 burchak qaytaradi (-90 dan +90 gacha)
  if (y < -30) return 0;     // Tepa
  if (x > 30)  return 90;    // O'ng
  if (y > 30)  return 180;   // Past
  if (x < -30) return 270;   // Chap

  return gravity; // O'zgarmasa eski holat
}
...
void resetTime() {
  ...
  fill(getTopMatrix(), 64);           // cantidad de particulas en la Matriz, si se define un intervalo de minutos u horas el valor debe ser 60
...
void setup() {
  ...
  Serial.println("MPU6050 ulanmoqda...");
  mpu6050.begin();
  mpu6050.calcGyroOffsets(true); 
  
  Serial.println("MPU6050 muvaffaqiyatli ishga tushdi!"); // Agar shu yozuv chiqsa, datchik ulandi
 ...
 */
void loop() {
  // 1. Datchik ma'lumotlarini yangilash va koordinatalarni chiqarish
  mpu6050.update(); 
  Serial.print("Burchak X: "); Serial.print(mpu6050.getAngleX());
  Serial.print(" | Burchak Y: "); Serial.println(mpu6050.getAngleY());

  // --- BLUETOOTHNI TEKSHIRISH ---
  if (Serial.available() > 0) {
    ...
    if (input.length() > 0) {
      ...
      if (m > 0) {
        delayMinutes = m; 
        myBT.print("Taymer: "); myBT.print(m); myBT.println(" min"); 
        resetTime(); 
      }
    }
  }
  ...
  // Gravitatsiyani aniqlash (bu funksiya ichida ham update bor edi)
  gravity = getGravity();
  lc.setRotation((ROTATION_OFFSET + gravity) % 360);

  // Rejimlarni tekshirish
  if (mode == MODE_SETMINUTES) {
